...
impl Poller for OsPoller {
    fn new() -> io::Result<Self> {
        ...
        rk_info!("[KQUEUE] created kqueue fd={}", kqueue_fd);
        rk_info!("[KQUEUE] allocated events buffer len={}", events.len());
    ...
    fn wait(&mut self, timeout_ms: i32) -> io::Result<Vec<EventObject>> {
        ...
        rk_info!(
            "[KQUEUE] timeout_ms={}, events_len={}, timeout_ptr_is_null={}",
            timeout_ms,
            self.events.len(),
            timeout_ptr.is_null()
        );
        ...
        rk_info!("[KQUEUE] kevent returned nevents={}", nevents);
